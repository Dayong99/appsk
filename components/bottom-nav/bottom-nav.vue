<template>
  <view class="posif posi-blr0 h112r bgwhite cblack bts2r flex z500 cuping" :style="setColorStyle">
    <!-- 我的消息气泡 -->
    <!-- <text class="my-bubble" v-if="bubble">{{bubble}}</text> -->
    <!-- 列表遍历 -->
    <block v-for="(item,index) in parameter.list" :key="index" class="ctheme">
      <view class="flex-fitem flexc-jsc flex-aic" @tap="$emit('current', index)">
        <image :class="[ item.text ?'hw44r' : 'hw96r']" :src="current==index ? item.selectedIcon : item.icon" mode="widthFix"></image>
