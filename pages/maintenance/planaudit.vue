				...
				<view class="beamList" style="">
					...
					<view class="plancontent">
						<view v-for="child in item.deviceInspectionPlanSubList" class="planchildlist">
							 <view class="planterm">
								<text class="goplan" :class="{'colortype':child.inspectionStatus==0}">{{child.inspectionStatus_dictText}}</text>
							...
							<view class="contengbox">
								<view class="botmeg">
								 ...
								 <view class="routing" v-if="!child.inspectionResult" @click="searcgvaky('examine')">去检查</view>
...
<script>
 import {inspectplan} from '@/api/AlbumServer.js'
  export default {
	data(){
		return{
			mescroll:null,
	    ...
	    inspectplan({
	      ...
	      pageSize: mescroll.size,
			...
			if(mescroll.num==1){
				...
				this.$store.commit('album/inspectplanList',this.getinspectplan.concat(violaRes.data.result.records))
			}
